...
const buttons = document.querySelectorAll('.main__nav-btn');
const heroImage = document.querySelector('.hero-content__image img');
const heroName = document.querySelector('.hero-content__name');
const heroDescription = document.querySelector('.hero-content__description');

buttons.forEach((button) => {
  button.addEventListener('click', () => {
    const hero = heroes[button.textContent];
    if (hero) {
      heroImage.src = hero.image;
      heroName.textContent = hero.name;
      heroDescription.textContent = hero.description;

      buttons.forEach((btn) => btn.classList.remove('main__nav-btn--active'));
      button.classList.add('main__nav-btn--active');
    } else {
      console.error(`Hero not found for button text: ${button.textContent}`);
    }
  });
});
